...
class Command(BaseCommand):
    ...
    def get_skills_in_job(self, job_dict):
        ...
        for skill in Skill.objects.all():
            if skill.name in job_dict["requirements"] or skill.name in job_dict["description"]:
    ...
    def process_link(self, job_dict):
        ...
        job_dict["description"] = " ".join(
            [
                item.__str__() for item in soup.find('h2', text = 'Job Description').find_next_siblings('p')
            ]
        )
        ...
        job_dict["description"] += "<br>" + soup.find('h2', text = 'Restrictions').find_next_siblings('ul')[0].__str__()
        job_dict["workplace"] = soup.find('span', {'class' : 'listing-location'}).text
        job_dict["salary_range"] = 10
        job_dict["remote"] = True if ("remote" in job_dict["workplace"] or "Remote" in job_dict["workplace"]) else False
        try:
            job_dict["company_email"] = re.findall(
                ...
                soup.find(
                    'h2',
                    text = 'Restrictions'
                ).find_next_siblings('ul')[1].text.strip()
            )[0]
        except:
            job_dict["company_email"] = None
        job_dict["country"] = Country.objects.get_or_create(name="Worldwide")[0]
        job_dict["issue_number"] = link.replace("https://www.python.org/jobs/", "").replace("/", "")
    ...
    def handle(self, *args, **options):
        ...
        for job in self.jobs:
            ...
            ) and Job.objects.filter(issue_number=job_dict["issue_number"]).count() == 0:
